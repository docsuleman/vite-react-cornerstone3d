...
import { WorkflowStage, RootPointType, CenterlineData } from '../types/WorkflowTypes';
...
const ProperMPRViewport: React.FC<ProperMPRViewportProps> = ({
  ...
  useEffect(() => {
    ...
    if (centerlineDataProp && (currentStage === WorkflowStage.ANNULUS_DEFINITION || currentStage === WorkflowStage.MEASUREMENTS) && centerlineChanged) {
      ...
      if (currentStage === WorkflowStage.ANNULUS_DEFINITION &&
          centerlineDataProp.annulusPlaneIndex !== undefined &&
          centerlineDataProp.annulusPlaneIndex >= 0) {

        const annulusPlaneIndex = centerlineDataProp.annulusPlaneIndex;
        currentCenterlineIndexRef.current = annulusPlaneIndex;

        // Update locked focal point to the exact annulus plane position
        const annulusPoint = [
          centerlineDataProp.position[annulusPlaneIndex * 3],
          centerlineDataProp.position[annulusPlaneIndex * 3 + 1],
          centerlineDataProp.position[annulusPlaneIndex * 3 + 2]
        ] as Types.Point3;
  ...
  const initializeMPRViewport = async () => {
    ...
    try {
      ...
      // IMPORTANT: In ANNULUS_DEFINITION, centerline may or may not have annular plane yet (depends on timing)
      // In MEASUREMENTS, centerline MUST already have annular plane from previous stage
      ...
      if (currentStage === WorkflowStage.ANNULUS_DEFINITION && existingSpheres && existingSpheres.length >= 3) {
        setTimeout(() => {
          ...
          if (centerlineDataRef.current && centerlineDataRef.current.annulusPlaneIndex !== undefined && centerlineDataRef.current.annulusPlaneIndex >= 0) {
            ...
            centerlineData = CenterlineGenerator.generateFromRootPoints(
              existingSpheres.map((sphere, index) => {
                let type: any;
                if (existingSpheres.length === 3) {
                  type = index === 0 ? RootPointType.LV_OUTFLOW :
            ...
            // Store TEMPORARY centerline data for scrolling
            // This will be replaced when TAVIApp provides the modified centerline
            centerlineDataRef.current = centerlineData;
          }
      ...
      if (currentStage === WorkflowStage.MEASUREMENTS && lockedFocalPointRef.current && centerlineDataRef.current) {
        ...
        // CRITICAL SAFETY CHECK: In MEASUREMENTS stage, centerline MUST have annular plane modification
        // This is medical software - we cannot use fallback data or regenerate centerlines
        if (centerlineDataRef.current.annulusPlaneIndex === undefined || centerlineDataRef.current.annulusPlaneIndex < 0) {
          const errorMsg = 'CRITICAL ERROR: Centerline does not have annular plane modification. Cannot proceed to MEASUREMENTS stage. Please return to ANNULUS_DEFINITION and re-place cusp points.';
          console.error(`🚨 ${errorMsg}`);
          setError(errorMsg);
          actions.addError(errorMsg);
          return;
        }
        ...
        setTimeout(() => {
          ...
          if (!centerlineData || centerlineData.annulusPlaneIndex === undefined || centerlineData.annulusPlaneIndex < 0) {
            const errorMsg = 'CRITICAL ERROR: Cannot position cameras - centerline annulus plane index not set. This should have been caught earlier.';
            console.error(`🚨 ${errorMsg}`);
            return; // Already showed error earlier, just return
          }
  ...
  const adjustToAnnularPlane = (dots: { id: string; pos: [number, number, number]; color: string; cuspType: string }[]) => {
    ...
    // CRITICAL FIX: Do NOT regenerate or overwrite centerlineDataRef here!
    // TAVIApp will generate the correct centerline WITH annular plane modification
    // and pass it via props. If we regenerate here, we overwrite the correct one.
    // Just use the existing centerlineDataRef for calculations (if needed).
    //
    // The modified perpendicular centerline will arrive from TAVIApp via the useEffect
    // at line 941, which updates centerlineDataRef.current from props.
    ...
    //   }))
    // );
    // centerlineDataRef.current = updatedCenterlineData;  // REMOVED - don't overwrite!

    // Recalculate scroll step size using EXISTING centerline (don't regenerate)
    // If centerline exists, use it; otherwise skip scroll step calculation
    if (centerlineDataRef.current) {
      const numPoints = centerlineDataRef.current.position.length / 3;
      let totalLength = 0;
      for (let i = 1; i < numPoints; i++) {
        const dx = centerlineDataRef.current.position[i * 3] - centerlineDataRef.current.position[(i - 1) * 3];
        const dy = centerlineDataRef.current.position[i * 3 + 1] - centerlineDataRef.current.position[(i - 1) * 3 + 1];
        const dz = centerlineDataRef.current.position[i * 3 + 2] - centerlineDataRef.current.position[(i - 1) * 3 + 2];
        totalLength += Math.sqrt(dx * dx + dy * dy + dz * dz);
      }
      const avgSegmentLength = totalLength / (numPoints - 1);
      const targetStepMM = 0.1; // 0.1mm per scroll event
